...
class UploadUtils:
    ...
    def initiate_multi_part_upload(
        ...
        coa_id: A string generated by API server
        error: A string of error message
        ------------------------------------------------------------------------------------------------------
        """
        data = dict(
            study_id_ext=row.study_id_ext,
        ...
        try:
            ...
            if "status_code" in res_json:
                if res_json["status_code"] == HTTPStatus.OK:
                    ...
                    return (res_json["upload_id"], res_json["coa_id"], error)
                if (
    ...
    def fetch_pre_signed_part_urls(
        url: str, coa_id: str, upload_id: str, file_path: str, headers
    ):
        """
        ------------------------------------------------------------------------------------------------------
        Class: UploadUtils
        ...
        try:
            ...
            url_to_get_presigned_url = f"{url}?type=presigned&coa_id={coa_id}&upload_id={upload_id}&number_of_parts={number_of_parts}"
    ...
    def upload(index, row, url, headers, force_upload) -> Tuple[bool, str]:
        ...
        ------------------------------------------------------------------------------------------------------
        """
        (upload_id, coa_id, error) = UploadUtils.initiate_multi_part_upload(
            index, row, url, headers, force_upload
        )
        filename = os.path.basename(row.file_path)
        uploaded = False
        if upload_id and coa_id:
            presigned_urls, number_of_parts = UploadUtils.fetch_pre_signed_part_urls(
                url, coa_id, upload_id, row.file_path, headers
            )
            if presigned_urls:
                if len(presigned_urls) != number_of_parts:
                    parts = json.dumps([])
                    res = requests.post(
                        f"{url}?type=complete&coa_id={coa_id}&upload_id={upload_id}&number_of_parts={number_of_parts}&filename={filename}",
                        json=parts,
                        headers=headers,
                    )
                else:
